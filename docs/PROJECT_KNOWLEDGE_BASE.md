...
### 2.1 The Orchestrator (`src/main.py`)
...
*   **Capabilities**:
    *   **Person Detection**: Uses `ultralytics` YOLOv8n model (`yolov8n.pt` from `data/`) on `Picamera2` frames.
...
### 2.4 The Voice: Audio Manager (`pyaudio_player.py`)

...
-   **Hybrid Low-Latency Q&A:** The system employs a three-tier architecture for answering user queries:
    1.  **Tier 1 (Instant):** A local keyword-matching system checks for common questions (greetings, locations, timings) defined in the knowledge base, providing immediate responses without internet latency.
    2.  **Tier 2 (Rule-Based):** Specific queries about hospital departments are handled by the rule-based navigation logic.
    3.  **Tier 3 (LLM Fallback):** Only if the first two tiers fail, the query is sent to the Llama 3.1 language model (via Groq API) for a generative response.
...
*   **Code Explanation:**
    *   **`InteractionProcess` Class:**
        ...
        *   `process_command`: The central dispatcher for user queries. It follows a strict priority:
            1.  **Keyword Search:** Checks for a matching question in the `interactions` knowledge base (Tier 1).
            2.  **Navigation/Rules:** Checks for department locations or specific policy lookups (Tier 2).
            3.  **LLM Fallback:** If no match is found, formats the query and sends it to `query_llama` (Tier 3).

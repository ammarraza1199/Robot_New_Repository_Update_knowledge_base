...
class InteractionProcess:
    ...
    def find_faq(self, user_text, lang):
        """
        Searches for FAQs using fuzzy matching on the question text.
        Implements Cross-Lingual Fallback: If not found in 'lang', checks 'en', 'hi', 'te'.
        """
            ...
            for faq in self.faqs:
                if "question" in faq and target_lang in faq["question"]:
                    ...
                    if user_text_lower in q_text or q_text in user_text_lower:
                        ...
                        if len(user_text_lower) < 5: # Short text requires exact match
        ...
        for fallback_lang in other_langs:
            ...
            if answer:
                logger.warning(f"--- FAQ MATCH (Fallback: {fallback_lang}) --- User sent '{lang}' but matched '{fallback_lang}'. Score: {score:.2f}")
                return answer # Return answer in the MATCHED language
    ...
    def start(self):
        ...
        while not self.shutdown_flag.is_set():
            try:
                ...
                logger.error(f"Error processing audio feedback queue: {e}")

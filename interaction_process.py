...
class InteractionProcess:
    ...
    def log_conversation(self, question, answer, language):
        with open(CONVERSATION_LOG_FILE, mode='a', newline='', encoding='utf-8') as f:
            csv.writer(f).writerow([datetime.now().isoformat(), language, question, answer])
    ...
    def process_command(self, command, lang):
        ...
        if any(re.search(r'\b' + re.escape(cmd) + r'\b', command) for cmd in self.EXIT_COMMANDS.get(lang, [])):
            ...
            item = self.find_process(command) or self.find_person(command) or self.find_location(command) or self.find_policy(command)
            if item:
                ...
                elif item['type'] == 'policy': response_text = self._format_policy_response(item, lang)
            else:
    ...
    def find_policy(self, text):
        for kw, policy_key in self.policy_keywords.items():
            if re.search(r'\b' + re.escape(kw) + r'\b', text.lower()):
                return {'type': 'policy', 'key': policy_key, 'value': self.policies.get(policy_key)}
        return None
    ...
    def find_department(self, text, lang):
        ...
        for kw, dept_cname in sorted_keywords:
            ...
            for match in re.finditer(r'\b' + re.escape(kw) + r'\b', processed_text):
    ...
    def _format_policy_response(self, data, lang):
        key = data.get('key', 'that policy').replace("_", " ")
        value = data.get('value', 'not specified')
        return f"Regarding {key}, the policy is: {value}."
    ...
    def is_medical_query(self, text):
        return any(kw in text for kw in ["symptoms", "causes", "treatment", "diagnose", "medicine", "lakshan", "ilaaj", "noppi", "mandhu"])

...
class MotorController:
    ...
    """
    def __init__(self, motor_queue, port='/dev/ttyAMA0', baud_rate=9600):
        self.motor_queue = motor_queue
        self.shutdown_flag = multiprocessing.Event()
        self.port = port
        self.baud_rate = baud_rate
        self.serial_connection = None
        self.last_command_time = 0.0 # Initialize last command time for rate limiting
        self.min_command_interval = 0.5 # Minimum interval between commands in seconds
    ...
    def handle_command(self, message):
        ...
        if not self.serial_connection:
            logger.warning("Motor command ignored: serial port not available.")
            return
        ...
        try:
            ...
            if time_since_last_command < self.min_command_interval:
                sleep_duration = self.min_command_interval - time_since_last_command
                logger.debug(f"Rate limiting: Sleeping for {sleep_duration:.3f} seconds before sending command.")
            ...
            if command == 'turn_to':
                ...
                else: # relative_angle is between -5 and 5
                    char_to_send = 'C' # Center, no movement or maintaining position
                    logger.info(f"Received relative angle {relative_angle}°, sending 'C' for Center (no significant movement).")

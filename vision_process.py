...
class VisionProcess:
    def __init__(self, audio_queue, audio_playing_flag, model_path='yolov8n.pt'):
        """
        ...
        if not cams:
            logger.warning("No camera detected. Vision subsystem disabled.")
        ...
        try:
            ...
            self.enabled = True # Only enable if both camera and model load successfully
    ...
    def queue_announcement(self, message):
        if self.audio_playing_flag.is_set():
            logger.info(f"Vision: Deferring announcement '{message}' because audio is already playing.")
            return
    ...
    def run(self, display=True):
        if not self.enabled:
            logger.error("VisionProcess is disabled. Entering sleep loop.")
            while True:
                time.sleep(5)
            return
...
def vision_process_func(audio_queue, shutdown_flag, audio_playing_flag, model_path='yolov8n.pt'):
    """Target function for the multiprocessing.Process."""
    setup_logging()
    logger.info("Setting up VisionProcess.")
    vision_system = VisionProcess(audio_queue, audio_playing_flag, model_path=model_path)
...
if __name__ == "__main__":
    ...
    try:
        vision_proc = VisionProcess(multiprocessing.Queue(), multiprocessing.Event()) 
        vision_proc.run()
